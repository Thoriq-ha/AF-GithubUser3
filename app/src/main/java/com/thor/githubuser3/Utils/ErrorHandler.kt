package com.thor.githubuser3.Utils
...
class ErrorHandler(private val application: Application) : Consumer<Throwable> {
    ...
    override fun accept(t: Throwable) {
        when (val cause = parseCause(t)) {
            is SocketTimeoutException, is ConnectException, is UnknownHostException, is SocketException -> {
                ...
                errorMessageToDisplay.onNext(application.getString(R.string.network_fail))
            }
            is HttpException -> {
